// ============================================
// account/core.js - Основные функции
// ============================================
...
function updateMobileAdminLinks() {
    ...
    if (menuLink) {
        menuLink.style.display = hasAccess ? 'block' : 'none';
    }
...
window.addEventListener('resize', () => {
    setTimeout(updateMobileAdminLinks, 100);
});
...
async function checkAuth() {
    ...
    if (s) {
        ...
        const dbRole = await getPlayerRole(currentUser.minecraft_nick || currentUser.username);
        if (!dbRole) {
            delete currentUser.role;
            const sd = JSON.parse(s);
            delete sd.role;
            localStorage.setItem('user_session', JSON.stringify(sd));
        } else {
        ...
        if (currentUser.role) {
            setTimeout(() => {
                showAdminPanelLink(currentUser.role);
                updateMobileAdminLinks();
            }, 100);
        } else {
            updateMobileAdminLinks();
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(updateMobileAdminLinks, 300);
    setTimeout(updateMobileAdminLinks, 600);
    setTimeout(updateMobileAdminLinks, 1000);
});
